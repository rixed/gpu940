...
static void draw_facet(unsigned f, bool ext, int32_t i_dec) {
	...
	for (unsigned v=4; v--; ) {
		...
		cmdVec[v].u.text.u = uvs[f][v][0];
		cmdVec[v].u.text.v = uvs[f][v][1];
		cmdVec[v].u.text.i = (i_dec-c3d[ cube_facet[f][v] ][2])<<6;
	}
...
static void transf_draw_pic(struct gpuBuf *pic_txt, FixVec *pic_vec, enum draw_what draw_what) {
	...
	static gpuCmdVector vecs[4] = {
		{ .same_as = 0, .u = { .text = { .u = 0<<16, .v = 0<<16 } }, },
		{ .same_as = 0, .u = { .text = { .u = 1<<16, .v = 0<<16 } }, },
		{ .same_as = 0, .u = { .text = { .u = 1<<16, .v = 1<<16 } }, },
		{ .same_as = 0, .u = { .text = { .u = 0<<16, .v = 1<<16 } }, }
	};
	static struct iovec cmdvec[] = {
